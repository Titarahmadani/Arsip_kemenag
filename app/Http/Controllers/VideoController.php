...
class VideoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = Video::when($katakunci, function ($query, $katakunci) {
        ...
        return view('video.viewvideo', ['Data' => $data, 'katakunci' => $katakunci]);
    }
    ...
    public function create()
    {
        ...
        $latest = Video::orderBy('created_at', 'desc')->first();
        $nextNumber = 'D01';
        if ($latest) {
        ...
        return view('video.tambahvideo', compact(
            'nextNumber',
            'unitPengolahan',
            'penciptaArsip',
            'lokasiArsip',
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            'nomor_arsip' => 'required|string|max:50|unique:video,nomor_arsip',
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('video.viewvideo')->with('success', 'Video berhasil ditambahkan.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            if ($request->hasFile('file')) {
                ...
                $video->file = $request->file('file')->store('arsip', 'public');
            }
            ...
            $video->update([
                ...
                'nomor_box' => $request->nomor_box,
                'keterangan' => $request->keterangan,
                'file' => $video->file, // tetap simpan path terbaru
            ]);

            DB::commit();
            return redirect()->route('video.viewvideo')->with('success', 'Video berhasil diperbarui.');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', 'Terjadi kesalahan: ' . $e->getMessage());
        }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('video.viewvideo')->with('success', 'Video berhasil dihapus.');
    }

    public function restore($id)
    {
        // Cari dokumen yang sudah di-soft-delete
        $dokumen = Video::withTrashed()->findOrFail($id);
        ...
        return redirect()->route('video.viewrestore')->with('success', 'Video berhasil dipulihkan.');
    }
}